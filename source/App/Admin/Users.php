...
class Users extends Admin
{
    ...
    public function home(?array $data): void
    {
        ...
        $all = ($search ?? "all");
        $paginator = new Pager(url("/admin/users/home/{$all}/"));
        $paginator->pager($users->count(), 12, (!empty($data["page"]) ? $data["page"] : 1));
        ...
        echo $this->view->render("widgets/users/home", [
            ...
            "head" => $head,
            "search" => $search,
            "users" => $users->order("first_name", "last_name")->limit($paginator->limit())->offset($paginator->offset())->fetch(true),
    ...
    public function user(?array $data): void
    {
        ...
        $head = $this->seo->render(
            CONF_SITE_NAME . " | " . ($userEdit ? "Perfil de {$userEdit->fullName()}" : "Novo Usuario"),
            CONF_SITE_DESC,
            url("/admin"),
            theme("/assets/images/image.jpg", CONF_VIEW_THEME),
            false
