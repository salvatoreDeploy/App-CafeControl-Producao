...
class Control extends Admin
{
    ...
    public function home()
    {
        ...
        echo $this->view->render("widgets/control/home", [
            ...
            "stats" => (object)[
                ...
                "recurrenceMonth" => (new AppSubscription())->recurrence(),
                "recurrence" => (new AppSubscription())->recurrenceMonth()
            ],
    ...
    public function subscriptions(?array $data): void
    {
        ...
        $all = ($search ?? "all");
        $paginator = (new Pager(url("admin/control/subscriptions/{$all}/")));
        $paginator->pager($subscriptions->count(), 12, (!empty($data["page"]) ? $data["page"] : 1));
        ...
        echo $this->view->render("widgets/control/subscriptions", [
            ...
            "head" => $head,
            "subscriptions" => $subscriptions->limit($paginator->limit())->offset($paginator->offset())->fetch(true),
    ...
    public function subscription(array $data) :void
    {
        ...
        if(!empty($data["action"]) && $data["action"] == "update"){
            ...
            if(!$subscriptionUpdate){
                $this->message->error("ERRO: Tente novamente ou atualize a pagina")->flash();
        ...
        if(!$subscription){
            $this->$this->message->error("Você tentou editar uma assinatura que não existe")->flash();
            redirect("admin/control/subscriptions");
        }
    ...
    public function plans(?array $data): void
    {
        ...
        $paginator = new Pager(url("/admin/control/plans/"));
        $paginator->pager($plans->count(), 5, (!empty($data["page"]) ? $data["page"] : 1));
        ...
        echo $this->view->render("widgets/control/plans", [
            "app" => "control/subscription",
            "head" => $head,
            "plans" => $plans->order("status ASC, created_at DESC")->limit($paginator->limit())->offset($paginator->offset())->fetch(true),
            "paginator" => $paginator->render()
        ]);
    }
    ...
    public function plan(?array $data): void
    {
        ...
        if(!empty($data["action"]) && $data["action"] == "delete"){
            ...
            if($planDelete->subscribers(null)->count()){
                $json["message"] = $this->message->warning("Não é possivel remover planos com assinaturas")->render();
                echo json_encode($json);
                return;
            }
