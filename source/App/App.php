...
class App extends Controller
{
    ...
    public function wallets(?array $data): void
    {
        ...
        if(!empty($data["wallet"]) && !empty($data["wallet_name"])){
            ...
            if(!$subscribe->count()){
                $this->message->error("Desculpe {$this->user->first_name}, para criar novas carteira é preciso ser PRO. Confira abaixo...")->flash();
        ...
        $head = $this->seo->render(
            "Minhas Cateiras - " . CONF_SITE_NAME,
            CONF_SITE_DESC,
            url(),
            theme("/assets/images/share.jpg"),
            false
