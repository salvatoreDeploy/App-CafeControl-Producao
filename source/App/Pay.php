...
class Pay extends Controller
{
    ...
    public function create(array $data): void
    {
        ...
        /*if (request_limit("paycreate", 5, 60 * 5)) {
        ...
        //Start Notification
        $image = "https://www.localhost/fsphp/themes/cafeadm/assets/images/notify.jpg";
        $title = "{$user->fullName()} assinou o plano {$plan->name} de R$ {$plan->price}/mês";
        $link = "https://www.localhost/fsphp/admin/control/subscription/{$subscription->id}";

        $notifications = (new Notification())->byNotifications($image, $title, $link);
        $notifications->save();
        //End Notification
    ...
    public function update(array $data): void
    {
        ...
        if(!$subscribe){
            $json["message"] = $this->message->error("Oooops! Você não tem uma assinatura ativa")->render();
            echo json_encode($json);
            return;
        }
        ...
        if($subscribe->status == "past_due"){
            ...
            if($transaction){
                ...
                $this->message->after(" e sua assinatura ja esta regularizada.");
